...
async function getDatamuseSuggestions(word: string): Promise<string[]> {
  try {
    ...
    const suggestions = [...spellingResults, ...soundResults]
      .map((result: DatamuseWord) => result.word)
      .filter(Boolean);
...
async function getSoundAlikeSuggestions(word: string): Promise<string[]> {
  try {
...
function getProfanityAlternatives(word: string): Promise<string[]> {
  // Check if the word might be an attempt at profanity
  const profanityList = profanity as Record<string, string[]>;
...
function isSimularToProfanity(input: string, profanity: string): boolean {
...
export function getWordAtPosition(text: string, position: number): { word: string; start: number; end: number } | null {
